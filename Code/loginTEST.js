(async function () {
  if (document.getElementById("bookmarklet-gui")) {
    ...
    fetch(
      "https://raw.githubusercontent.com/TheHumblePotato/Yap-Window/refs/heads/main/Code/gui.jsTEST?token=$(date +%s)",
    )
      .then((r) => r.text())
      .then((code) => {
        eval(code);
        ...
        async function openChatScreen() {
          ...
          fetch(
            "https://raw.githubusercontent.com/TheHumblePotato/Yap-Window/refs/heads/main/Code/chat.jsTEST?token=$(date +%s)",
          )
            .then((r) => r.text())
            .then((chatCode) => {
